...
async function relatorioFicha() {
  ...
  ipcMain.on('buscar-cliente-nome', async (event, nome) => {
    try {
      const cliente = await clientModel.findOne({ nomeCliente: new RegExp(nome, 'i') });
      if (!cliente) {
        event.reply('cliente-nao-encontrado');
      } else {
        event.reply('cliente-encontrado', cliente);
      }
    } catch (error) {
      console.error(error);
      event.reply('erro-busca-cliente');
    }
  });

  ipcMain.on('salvar-ficha', async (event, ficha) => {
    try {
      const novaFicha = new assistenciaModel(ficha);
      await novaFicha.save();
      dialog.showMessageBox({ message: 'Ficha salva com sucesso!', type: 'info' });
      event.reply('reset-form-assistencia');
    } catch (error) {
      console.error('Erro ao salvar ficha:', error);
      dialog.showMessageBox({ message: 'Erro ao salvar ficha.', type: 'error' });
    }
  });
